    ...
    _.each(_.keys(toggleLabels), function(toggleName) {
      ...
      element =
          $("<li></li>")
              ...
              .append(
                  $("<div></div>").addClass("toggle").append($("<div></div>")))
              .append($("<div></div>").addClass("title").text(element));
...
function updateSpecDisplay(shipUI, attrName, value) {
  var specElement;
  (specElement = shipUI.elements.specs[attrName]) &&
      specElement.find("span").text(value)
}

function updateToggleDisplay(shipUI, toggleName, isOn) {
  var toggleElement;
  (toggleElement = shipUI.elements.toggles[toggleName]) &&
      (isOn ? toggleElement.find("div.toggle").addClass("on")
            : toggleElement.find("div.toggle").removeClass("on"))
}
...
function renderShip(shipUI) {
  ...
  updateToggleDisplay(shipUI, "point_defense", shipUI.data.pointDefense)
}

function getShipData(shipUI) {
  return {
    name: shipUI.data.name, shipClass: shipUI.data.shipClass,
        number: shipUI.data.number, hull: shipUI.data.hull,
        yellow: shipUI.data.yellow, orange: shipUI.data.orange,
...
function loadPresetsFromCookie(manager) {
  ...
  if (cookieData)
    try {
      ...
      manager.customPresets = _.filter(manager.customPresets,
                                       function(preset) { return preset.name })
    } catch (error) {
...
function savePresetsToCookie(manager) {
  ...
  $.cookie("presets", cookieData, {expires : 365})
}
...
function removePreset(manager, presetName, element) {
  ...
  preset && (manager.customPresets = _.without(manager.customPresets, preset),
             savePresetsToCookie(manager), element.hideAnimated())
}
...
function renderPresetList(manager) {
  ...
  _.each(manager.defaultPresets, function(preset) {
    preset = $("<li></li>")
                 .addClass(preset.type)
                 .addClass(preset.shipClass.toLowerCase())
                 .attr("rel", preset.name)
                 .append($('<div class="icon"></div>')
                             .append($("<span></span>").text(preset.name)));
    list.append(preset)
  });
  _.each(manager.customPresets, function(preset) {
    preset = $("<li></li>")
                 ...
                 .append($('<div class="icon"></div>')
                             .append($("<span></span>").text(preset.name)))
                 .append($("<div>&ndash;</div>").addClass("remove"));
    list.append(preset)
  })
}
...
function findPresetByName(manager, presetName) {
  if (presetName) {
    ...
    return customPreset
               ? customPreset
               : _.find(manager.defaultPresets, function(preset) {
                   return preset.name.toLowerCase() == presetName.toLowerCase()
                 })
  }
}
...
$(function() {
  ...
  function displayResults(results) {
    ...
    $("div.results .victorychance .attacker .probability")
        .text(Math.round(100 * results.attacker) + "%");
    results.attacker > results.defender
        ? $("div.results .victorychance div.attacker").addClass("winner")
    ...
    _.each(results.shipsAttacker, function(survivalRate, shipName) {
      var listItem = $(
          '<li><div class="class"></div><div class="name"></div><div class="probabilities"><span class="chance1"></span></div></li>');
      listItem.find(".name").text(shipName);
      listItem.find(".probabilities .chance1")
  ...
  function showPresetLightbox(shipUI, removeMode) {
    $("div.presets").data({targetShip : shipUI, removeMode : removeMode});
    $(".lightbox#presets").fadeIn()
  }

  function hidePresetLightbox() { $(".lightbox#presets").fadeOut() }
  ...
  function applyPreset(presetName) {
    ...
    targetShip && presetName && preset
        ? (setShipData(targetShip, preset), renderShip(targetShip),
           hideResults())
        : removeMode && removeShip(targetShip.elements.container)
  }

  function deferExecution(fn) { setTimeout(fn, 0.2) }

  function updateSimulateButton() {
    0 < $("div#shipsD ul.ships li.ship").length &&
            0 < $("div#shipsA ul.ships li.ship").length
        ? $("a.simulate").removeClass("disabled")
        : $("a.simulate").addClass("disabled")
  }
  ...
  $(document.body).on("click", "ul.specs li", function() {
    ...
    "yellow" == attrName
        ? (shipUI.data.yellow++,
           shipUI.data.yellow > ATTRIBUTE_LIMITS.yellow[1] &&
               (shipUI.data.yellow = ATTRIBUTE_LIMITS.yellow[0]))
    : "orange" == attrName
        ? (shipUI.data.orange++,
           shipUI.data.orange > ATTRIBUTE_LIMITS.orange[1] &&
               (shipUI.data.orange = ATTRIBUTE_LIMITS.orange[0]))
    : "blue" == attrName ? (shipUI.data.blue++,
                            shipUI.data.blue > ATTRIBUTE_LIMITS.blue[1] &&
                                (shipUI.data.blue = ATTRIBUTE_LIMITS.blue[0]))
    : "red" == attrName
        ? (shipUI.data.red++, shipUI.data.red > ATTRIBUTE_LIMITS.red[1] &&
                                  (shipUI.data.red = ATTRIBUTE_LIMITS.red[0]))
    : "missiles_yellow" == attrName
        ? (shipUI.data.missiles_yellow++,
           shipUI.data.missiles_yellow > ATTRIBUTE_LIMITS.missiles_yellow[1] &&
               (shipUI.data.missiles_yellow =
                    ATTRIBUTE_LIMITS.missiles_yellow[0]))
    : "missiles_orange" == attrName
    ...
    : "missiles_red" == attrName
        ? (shipUI.data.missiles_red++,
           shipUI.data.missiles_red > ATTRIBUTE_LIMITS.missiles_red[1] &&
               (shipUI.data.missiles_red = ATTRIBUTE_LIMITS.missiles_red[0]))
    : "computers" == attrName
        ? (shipUI.data.computers++,
           shipUI.data.computers > ATTRIBUTE_LIMITS.computers[1] &&
               (shipUI.data.computers = ATTRIBUTE_LIMITS.computers[0]))
    : "shields" == attrName
        ? (shipUI.data.shields++,
           shipUI.data.shields > ATTRIBUTE_LIMITS.shields[1] &&
               (shipUI.data.shields = ATTRIBUTE_LIMITS.shields[0]))
    : "hull" == attrName ? (shipUI.data.hull++,
                            shipUI.data.hull > ATTRIBUTE_LIMITS.hull[1] &&
                                (shipUI.data.hull = ATTRIBUTE_LIMITS.hull[0]))
    : "initiative" == attrName
        ? (shipUI.data.initiative++,
           shipUI.data.initiative > ATTRIBUTE_LIMITS.initiative[1] &&
               (shipUI.data.initiative = ATTRIBUTE_LIMITS.initiative[0]))
        : "number" == attrName &&
              (shipUI.data.number++,
               shipUI.data.number > ATTRIBUTE_LIMITS.number[1] &&
                   (shipUI.data.number = ATTRIBUTE_LIMITS.number[0]));
    renderShip(shipUI);
    hideResults(!0);
    return !1
  });
  ...
  $(document.body).on("click", "ul.toggles li", function() {
    ...
    "splitter" == toggleName ? shipUI.data.splitter = !shipUI.data.splitter
    : "missile_shield" == toggleName
        ? shipUI.data.missile_shield = !shipUI.data.missile_shield
        : "point_defense" == toggleName &&
              (shipUI.data.pointDefense = !shipUI.data.pointDefense);
    renderShip(shipUI);
    hideResults(!0);
    return !1
  });
  ...
  $(document.body).on("click", "a.templates", function() {
  ...
  $(document.body).on("click", "a.save", function() {
    var shipUI = $(this).parents("li.ship").data();
    var presetName = window.prompt("Name the preset",
                                   "Ship " + Math.ceil(1E3 * Math.random()));
    if (!presetName)
      return !1;
  ...
  $(document.body).on("click", "a.simulate", function() {
    var attackFleet = {ships : []};
    var defenseFleet = {ships : []};
    ...
    _.size(defenseFleet.ships) && _.size(attackFleet.ships) &&
        $(".lightbox#working").fadeIn("swing", function() {
