function Dice(diceType, originShip, isMissile) {
  this.damageValue = "red" == diceType      ? 4
                     : "blue" == diceType   ? 3
                     : "orange" == diceType ? 2
                                            : 1;
  this.type = diceType;
...
function checkHit(dice, target) {
  ...
  return (6 <= dice.value + dice.origin.computers - shields ||
          6 == dice.value) &&
         1 != dice.value
}
...
function splitAntimatter(dicePool) {
  var redDice =
      _.filter(dicePool.dice, function(dice) { return "red" == dice.type });
  ...
  function hasHittingDice() {
    return _.find(sortedDice, function(dice) {
      return _.find(
          targets,
          function(
              target) { return target.isAlive() && checkHit(dice, target) })
    })
  }

  var sortedDice =
      _.sortBy(dicePool.dice,
               function(dice) { return 100 * dice.value - dice.damageValue });
  var prioritizedTargets = _.sortBy(
      targets,
      function(
          target) { return -target.targetPriority.indexOf(target.shipClass) });
  for (hasHittingDice(); hasHittingDice();) {
    ...
    var killableTarget = _.find(prioritizedTargets, function(target) {
      return target.isAlive() &&
             canBeKilled(target, _.union(sortedDice, [ currentDice ])) &&
...
function Combatant(config) {
  this.shipClass =
      config.shipClass || "Unknown " + Math.round(100 * Math.random());
...
Combatant.prototype.targetPriority =
...
Combatant.prototype.isAlive = function() { return 0 < this.hp };
function canBeKilled(target, incomingDice) {
  var hittingDice =
      _.filter(incomingDice, function(dice) { return checkHit(dice, target) });
  return _.reduce(hittingDice,
                  function(totalDamage,
                           dice) { return totalDamage + dice.damageValue },
                  0) >= target.hp
}
function ShipGroup(shipConfig, isDefender) {
  ...
  for (var shipCount = shipConfig.number || 1, i = 0; i < shipCount; i++) {
    ...
    isDefender ? (ship.initiative += 0.1, ship.side = "D") : ship.side = "A";
    this.ships.push(ship)
  }
}

ShipGroup.prototype.isEmpty =
    function() { return !this.ships || 0 == _.size(this.ships) };
...
function getLivingShips(shipGroup) {
  return _.filter(shipGroup.ships, function(ship) { return ship.isAlive() })
}
ShipGroup.prototype.hasLivingShips =
    function() { return 0 < _.size(getLivingShips(this)) };
function Initiative(defenderFleet, attackerFleet) {
  ...
  _.each(
      attackerFleet.ships,
      function(
          shipConfig) { self.combatants.push(new ShipGroup(shipConfig, !1)) });
  self.combatants = _.sortBy(
      self.combatants,
      function(shipGroup) { return -getFirstShip(shipGroup).initiative })
}

function getAllCombatants(initiativeOrder) {
  var allShips = [];
  _.each(initiativeOrder.combatants,
         function(shipGroup) { allShips = _.union(allShips, shipGroup.ships) });
  return allShips
}

function getLivingCombatantsBySide(initiativeOrder, side) {
  return _.filter(
      getAllCombatants(initiativeOrder),
      function(ship) { return ship.isAlive() && (ship.side == side || !side) })
}
Initiative.prototype.status = function() {
  ...
  var hasWeapons = _.find(
      getLivingCombatantsBySide(this),
      function(
          ship) { return 0 < ship.yellow || 0 < ship.orange || 0 < ship.red });
  status.battleOver =
      !status.attackerAlive || !status.defenderAlive || !hasWeapons;
  status.battleOver && (status.winner = status.defenderAlive ? "D" : "A");
  if (status.battleOver) {
    var livingShips = _.countBy(
        getLivingCombatantsBySide(this),
        function(ship) { return ship.side == status.winner ? ship.name : "" });
    var totalShips = _.countBy(
        getAllCombatants(this),
        function(ship) { return ship.side == status.winner ? ship.name : "" });
...
function resolveBattle(defenseFleet, attackFleet) {
  var result = {winner : "", shipSurvival : {}};
  ...
  for (var battleStatus = fireOrder.status(); !battleStatus.battleOver;)
    combatRound(fireOrder, !1), battleStatus = fireOrder.status();
...
function combatRound(fireOrder, missileOnly) {
  ...
  _.each(fireOrder.combatants, function(shipGroup) {
    if (shipGroup.hasLivingShips()) {
      ...
      var enemyShips = _.filter(
          allShips,
          function(
              ship) { return ship.side != attackerSide && ship.isAlive() });
...
function aggregateSurvivalStats(battleResults) {
  ...
  _.each(battleResults, function(result) {
    _.each(result.shipSurvival, function(survivalRate, shipName) {
      totals[shipName] =
          totals[shipName] ? totals[shipName] + survivalRate : survivalRate
    })
  });
  var averages = {};
  _.each(totals,
         function(
             total,
             shipName) { averages[shipName] = total / _.size(battleResults) });
  return averages
}

function calculate(defenseFleet, attackFleet, iterations) {
  var res =
      {attacker : 0, defender : 0, shipsAttacker : {}, shipsDefender : {}};
  for (var results = [], i = 0; i < iterations; i++) {
    var battle = resolveBattle(defenseFleet, attackFleet);
    results.push(battle)
  }
  var defenderWins =
      _.filter(results, function(result) { return "D" == result.winner });
  var attackerWins =
      _.filter(results, function(result) { return "A" == result.winner });
...
window.AnalyzeBattle = function(
    defenseFleet, attackFleet,
    iterations) { return calculate(defenseFleet, attackFleet, iterations) };
